...
def draw_stacks(stacks: list[list[str]], highlights=None):
    ...
    for col, stack in enumerate(stacks):
        for row, letter in enumerate(stack):
            ...
            draw.text((x0 + ADJUST_X, y0 + ADJUST_Y), letter, BACKGROUND)
...
class Instruction:
    ...
    def execute(self, stacks: list[list[str]]):
        ...
        highlights = set(
            map(lambda y: (fr, y), range(remlen - self.move_count, remlen))
        )
        highlights.update(
            map(lambda y: (to, y), range(addlen, addlen + self.move_count))
        )
